...
const MasterPage = ({children, className}) => {
  ...
  return (<>
    ...
    <div className="flex flex-row bg-gray-900 p-4">
      <MenuOutlined style={{ fontSize: 32, marginRight: 20, color: '#f1f5f9' }} onClick={toogleMenu} />
      <div className="logo text-slate-100">
        MTG Limited
      </div>
    </div>
    <div className="flex flex-row">
      <div className={`${menuVisibility(menuState)} bg-gray-900 text-slate-100 h-screen w-[190px] absolute top-[67px]`}>
        <ul>
          <li className="py-4 pl-4">
            <a onClick={launchClassifier}>
              <TagOutlined style={{ fontSize: 20, marginRight: 8, color: '#f1f5f9' }} />
              Classifier game
            </a>
          </li>
        </ul>
